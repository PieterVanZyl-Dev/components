...
interface ContributedTableProps<T> {
      ...
      >
        {columnDefinitions.map((col, index) => (
          <Table.DataCell key={col.id}>
            ...
            <span style={index === 0 ? { paddingLeft: `${15 * (level - 1)}px` } : {}}>
              {index === 0 && expandable ? (
                <Button
                  variant="icon"
...
function ContributedExpandableTable<T>({ columnDefinitions, items, trackBy }: ContributedTableProps<NestedItem<T>>) {
...
export default function () {
  return (
    <>
      <h1>Contributed table</h1>

      <ScreenshotArea disableAnimations={true}>
        <ContributedExpandableTable<{ name: string; key1: string; key2: string }>
          columnDefinitions={[
